...
public class ICSParser {
    ...
    public static EDTCalendar readICS(String link) throws IOException {
        ...
        while ((line = reader.readLine()) != null) {
            if (line.startsWith("BEGIN:")) {
            ...
            else if (line.startsWith("DESCRIPTION") || line.startsWith("X-ALT-DESC") || line.startsWith("CATEGORIES:") || line.startsWith("DTSTAMP:")) {
                reader.reset();
                readLines(reader);
                continue;
            }
                ...
                if (summary.length > 1) {
                    reservation.setNameOfReservation(summary[0]);
                    reservation.setTeachers(new ArrayList<String>(List.of(summary[1].split("\\\\"))));
                    reservation.setAttendingGroups(new ArrayList<String>(List.of(summary[2].split("\\\\"))));
                    reservation.setType(summary[3]);
                    if (summary.length == 5) {
                        reservation.setMemo(summary[4]);
                    }
                }
                else {
